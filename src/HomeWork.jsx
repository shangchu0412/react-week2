...
function HomeWork() {
  ...
  const addToCart = (drink) => {
    const repeat = cart.find((item) => item.id === drink.id)
    if (repeat) {
      const updataRepeat = cart.map((item) => {
        return item.id === drink.id && item.quantity < 10 ? {
          ...item,
  ...
  return (
    <div className="container mt-5">
            ...
            {drink.map(drink => {
              return (
                <a href="#" className="list-group-item list-group-item-action" key={drink.id}
                  ...
                  <p className="mb-1">{drink.description}</p></a>
              )
            })}
          </div>
        </div>
      ...
      <div className="row justify-content-center">
        <div className="col-8">
          {!order.id ? <div className="alert alert-secondary text-cent" role="alert">
            尚未建立訂單
          </div> : (
            <div className="card">
